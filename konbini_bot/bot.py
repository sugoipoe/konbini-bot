...
from gpt import ask_gpt, ask_gpt_session, ask_gpt_result
...
async def manage_voice_session(user, voice_client):
    ...
    while True:
        ...
        if "ありがとう" in response:
            gpt_result = await ask_gpt_result(sessions.get(voice_client.channel.name))
            results = gpt_result.split(',')
            await channel.send(f"Results: {results}")
